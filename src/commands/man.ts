...
export function cmdMan(args: string, ctx: CommandContext): { output: string; error?: boolean } {
  ...
  if (args.toLowerCase() === 'ryan') {
    ...
    const header = mobile
      ? `<span class="tc-yellow">RYAN(1)</span>  <span class="tc-muted">User Commands</span>`
      : `<span class="tc-yellow">RYAN(1)</span>                    <span class="tc-muted">User Commands</span>                    <span class="tc-yellow">RYAN(1)</span>`;
    if (mobile) {
      return { output: `${header}

<span class="tc-yellow">NAME</span>
  ryan — husband, builder, founder,
  open-source contributor

<span class="tc-yellow">SYNOPSIS</span>
  <span class="tc-white">ryan</span> [--build] [--break]
       [--fix] [--repeat]

<span class="tc-yellow">DESCRIPTION</span>
  Partner & CIO at ${click('Oodle', 'cat projects/oodle', 'tc-link-inline')},
  where he built ${click('Herald', 'cat projects/herald', 'tc-link-inline')}.
...
<span class="tc-yellow">OPTIONS</span>
  ...
  <span class="tc-white">--dogs</span>     ${click('Remus', 'cat about/dogs/remus', 'tc-link-inline')} + ${click('Arthas José', 'cat about/dogs/arthas', 'tc-link-inline')}
  <span class="tc-white">--verbose</span>  Has opinions.
  <span class="tc-white">--edge</span>     Runs on edge.
...
<span class="tc-yellow">BUGS</span>
  First website still online because
  he forgot the password. Cannot stop
  building things. Reformed Apple
  fanboy — relapse unlikely but not
  impossible.

<span class="tc-yellow">SEE ALSO</span>
  ${click('resume.txt', 'cat resume.txt', 'tc-link-inline')}, ${click('connect/', 'cat connect/', 'tc-link-inline')}
  <a href="https://heyoodle.com" target="_blank" rel="noopener noreferrer" class="tc-link">heyoodle.com</a>, <a href="https://x.com/ryanrhughes" target="_blank" rel="noopener noreferrer" class="tc-link">@ryanrhughes</a>` };
    }
...
<span class="tc-yellow">DESCRIPTION</span>
       Partner & CIO at ${click('Oodle', 'cat projects/oodle', 'tc-link-inline')}, where he built ${click('Herald', 'cat projects/herald', 'tc-link-inline')}.
       Core team on ${click('Omarchy', 'cat projects/omarchy', 'tc-link-inline')}. Runs ${click('luxury vacation rentals', 'cat projects/sunset-villas', 'tc-link-inline')} near
...
<span class="tc-yellow">SEE ALSO</span>
       ${click('resume.txt', 'cat resume.txt', 'tc-link-inline')}, ${click('connect/', 'cat connect/', 'tc-link-inline')}, <a href="https://heyoodle.com" target="_blank" rel="noopener noreferrer" class="tc-link">heyoodle.com</a>, <a href="https://x.com/ryanrhughes" target="_blank" rel="noopener noreferrer" class="tc-link">@ryanrhughes</a>` };
  }
