...
The other thing that clicks once you have this hardware running: inference isn't the only thing you can do with it. Open-weight models mean you can train on your own data — fine-tune for your specific domain, build something the APIs will never give you. That's a whole different post, but it changes how you think about the investment.

I'm going to keep running Nemotron alongside my frontier models and see where the actual boundary settles. The interesting thing isn't that local models are catching up to frontier — they're not, at least not on a single GPU. (Models like [Kimi K2.5](https://github.com/MoonshotAI/Kimi-K2) are genuinely frontier-competitive, but you need 8x GPUs to run them. Different conversation.) It's that "good enough" covers way more ground than I expected.
